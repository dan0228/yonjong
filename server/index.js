  ...
  socket.on('requestFriendMatchmaking', async ({ userId, passcode, username, avatarUrl, actionType }) => {
    ...
    try {
        ...
        if (players && Array.isArray(players)) {
            if (out_is_full) {
                // ゲームが満員になった場合、全てのプレイヤーに game-found イベントを送信
                io.to(out_game_id).emit('game-found', { gameId: out_game_id, players: players });
            } else {
                // それ以外の場合、マッチメイキングの更新を全てのプレイヤーにブロードキャスト
                io.to(out_game_id).emit('matchmaking-update', { gameId: out_game_id, players: players });
            }
        } else {
